...
function next() {
	...
	if (document.getElementById("hiragana").checked && document.getElementById("katakana").checked) {
		if (Math.floor((Math.random()*2)+1) == 1) {
			document.getElementById("kana").innerHTML = hiragana.get(romaji[choice]);
		} else {
			document.getElementById("kana").innerHTML = katakana.get(romaji[choice]);	
		}
...
function verify() {
	...
	if (kana == hiragana.get(input) || kana == katakana.get(input)) {
		next();
	} else {
